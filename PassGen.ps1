	...
	switch ($wordlist) {
		"Simple" {
			if ($PSScriptroot) {
				...
				$dictionary=Get-Content -Path "H:\Scripts\Utilities\Ogden_basic_dictionary.txt" | ? {$_.Length -gt 4}
			}
		...
		"Common" {
			if ($PSScriptroot) {
				...
				$dictionary=Get-Content -Path "H:\Scripts\Utilities\MIT_common_dictionary.txt"
			}
			[boolean]$complex=$false
			[boolean]$global:special=$true
		}
		"Strong" {
			if ($PSScriptroot) {
				...
				$dictionary=Get-Content -Path "H:\Scripts\Utilities\EFF_large_dictionary.txt"
			}
...
function New-Char {
	...
	switch ($chartype) {
		"Lower" {
		    $seed=Get-Random -minimum 0 -maximum $global:lowerpool.count
		    $global:password+=$global:lowerpool[$seed]
		}
		"Upper" {
		    $seed=Get-Random -minimum 0 -maximum $global:upperpool.count
			$global:password+=$global:upperpool[$seed]
		}
		"Int" {
			$global:password+=(Get-Random -Minimum 10 -Maximum 100)
		}
		"Special" {
		    $seed=Get-Random -minimum 0 -maximum $global:specialpool.count
			$global:password+=$global:specialpool[$seed]
		}
	}
	Remove-Variable -Name seed -force -erroraction silentlycontinue
}
...
$global:lowerpool=@("a","b","c","d","e","f","g","h","i","j","k","m","n","o","p","q","r","s","t","u","v","w","x","y","z")
$global:upperpool=@("A","B","C","D","E","F","G","H","J","K","L","M","N","P","Q","R","S","T","U","V","W","X","Y","Z")
$global:specialpool=@("(",")","!","@","$","&","*","-","+","=",".","?")
...
$button3.Text="New strong passphrase"
...
$PasswordLabel.location=New-Object System.Drawing.Point(0,120)
